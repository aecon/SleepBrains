...
print("min Volume>=%d and Intensiy Ratio>=%d:" % (THRES_vmin, THRES_i), np.sum( (volumes>=THRES_vmin)*(IntensityRatio>=THRES_i))/len(labels) )
...
plt.subplot(2,3,6)
plt.hist(volumes, Nbins*10, log=True)
plt.xlabel('volume')
plt.xscale('log')
