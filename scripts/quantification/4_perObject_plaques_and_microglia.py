...
def count_points_in_region(regions, indices, volumes, psum, microgliacounts, Nregions):
    ...
    microglia_volume_per_region = np.zeros(Nregions, dtype=np.float32)
    for pi in range(Np):
        i = indices[pi,0]
        j = indices[pi,1]
        k = indices[pi,2]
        if ((i<nx) and (j<ny) and (k<nz)):  # exclude points outside Atlas space
            ...
            if region_id > 0:  # if not background
                ...
                microglia_volume_per_region[region_id-1] += microgliacounts[pi] * dx*dx*dz # in mm3
    ...
    return counts_per_region, volumes_per_region, total_prob_per_region, region_volumes, microglia_volume_per_region
...
if __name__ == "__main__":
    ...
    for file_centroids in paths_centroids:
        ...
        file_microglia = "/media/user/SSD1/Athena/Data/PROJECT_SLEEP_2024-09/ILASTIK/microglia_counts_in_plaques/counts_plaque_associated_microglia_Brain%02d.dat" % brainID
        print(file_microglia)
        ...
        # Load microglia counts
        mdata = np.loadtxt(file_microglia, skiprows=1)
        mcounts = mdata[:,1]

        # Get number of points, volumes and associated probabilities
        counts_per_region, volumes_per_region, total_prob_per_region, region_volumes, microglia_volume_per_region = count_points_in_region(regions, indices, volumes, psum, mcounts, Nregions)
        ...
        output_file = basedir + os.sep + "Brain%02d_region_plaquesAndMicroglia_%s" % (brainID, channel) + brainid + "_%s.csv" % kind
        print(output_file)
        with open(output_file, "w") as f:
            f.write("%s,%s,%s,%s,%s,%s\n" % ("Region","ObjCounts","Concentration(V/V)","TotalProbabilityOverRegionNpixels","MeanPlaqueSize(Um3)","VolMicroglia/VolPlaque") )
            for i in range(Nregions):
                f.write("%s,%d,%.6e,%.6e,%.6e,%.6e\n" % (brain_regions_include[i], counts_per_region[i], volumes_per_region[i]/region_volumes[i], total_prob_per_region[i]*dx*dx*dz/region_volumes[i], volumes_per_region[i]/float(counts_per_region[i])*1.e3*1.e3*1.e3, microglia_volume_per_region[i]/volumes_per_region[i] ))
